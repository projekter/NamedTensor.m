...
SplitRowIndices::usage="SplitRowIndices[tensor,index->{{name1,dimension1},...},...] splits the row index specified in `index` into multiple new indices with given dimensions. Negative dimensions are interpreted as placeholders: after assigning the positive dimensions, the remaining dimensions are distributed according to the negative exponents (-2 is the square of -1).

SplitRowIndices[tensor,index->{name1,name2,...}] automatically assigns equally distributed dimensions (as `name1->-1,name2->-1,...` would do).
SplitRowIndices[tensor,index->{dimension1,dimension2,...}] automatically assigns new names `index.1`, `index.2`, ....
SplitRowIndices[tensor,index->count] automatically splits into `count` equally distributed indices with autogenerated names.";
SplitColumnIndices::usage="SplitColumnIndices[tensor,index->{{name1,dimension1},...},...] splits the columm index specified in `index` into multiple new indices with given dimensions. Negative dimensions are interpreted as placeholders: after assigning the positive dimensions, the remaining dimensions are distributed according to the negative exponents (-2 is the square of -1).

SplitColumnIndices[tensor,index->{name1,name2,...}] automatically assigns equally distributed dimensions (as `name1->-1,name2->-1,...` would do).
SplitColumnIndices[tensor,index->{dimension1,dimension2,...}] automatically assigns new names `index.1`, `index.2`, ....
SplitColumnIndices[tensor,index->count] automatically splits into `count` equally distributed indices with autogenerated names.";
SplitIndices::usage="SplitIndices[tensor,index->{{name1,dimension1},...},...] splits the index specified in `index` into multiple new indices with given dimensions for both row and column indices. Negative dimensions are interpreted as placeholders: after assigning the positive dimensions, the remaining dimensions are distributed according to the negative exponents (-2 is the square of -1).

SplitIndices[tensor,index->{name1,name2,...}] automatically assigns equally distributed dimensions (as `name1->-1,name2->-1,...` would do).
SplitIndices[tensor,index->{dimension1,dimension2,...}] automatically assigns new names `index.1`, `index.2`, ....
SplitIndices[tensor,index->count] automatically splits into `count` equally distributed indices with autogenerated names.

SplitIndices[tensor,{rowRule1,...},{columnRule1,...}] allows to specify different splitting rules for row and column indices.";
...
SplitIndices`subvalidate[_String->_Integer]:=True;
SplitIndices`subvalidate[{_String,_Integer}]:=True;
SplitIndices`validate[_String->_List?(VectorQ[#,SplitIndices`subvalidate]&)]:=True;
SplitIndices[NamedTensor[rowNames_Association,colNames_Association,data_],rowSplitting_List?(VectorQ[#,SplitIndices`validate]&),colSplitting_List?(VectorQ[#,SplitIndices`validate]&)]:=
  Catch@Module[{allNewNames={rowNames,colNames},newShape=TensorDimensions[data]},
    Do[
      With[{oldName=splitting[[1]],newNames=splitting[[2,All,1]],\[Delta]=Length@splitting[[2]]-1,newWhichNames=allNewNames[[which[[2]]]],originalWhichNames=which[[3]],j=which[[2]]},
        With[{oldIndex=newWhichNames[oldName],oldDimension=newShape[[newWhichNames[oldName]]]},
          With[{newDimensions=splitting[[2,All,2]]/.i_?Negative:>(oldDimension/Times@@DeleteCases[splitting[[2,All,2]],_?Negative])^(-i/Total@Ramp[-splitting[[2,All,2]]])},
            If[\[Delta]<0 || !KeyExistsQ[originalWhichNames,oldName] || !DuplicateFreeQ[newNames] ||
              AnyTrue[newNames,#=!=oldName&&KeyExistsQ[newWhichNames,#]&] || !AllTrue[newDimensions,IntegerQ] ||
             Times@@newDimensions=!=newShape[[oldIndex]],
               Throw[$Failed]
            ];
            KeyValueMap[If[#2>oldIndex,allNewNames[[1,Key[#1]]]+=\[Delta];]&,allNewNames[[1]]];
            KeyValueMap[If[#2>oldIndex,allNewNames[[2,Key[#1]]]+=\[Delta];]&,allNewNames[[2]]];
            KeyDropFrom[allNewNames[[j]],oldName];
            AssociateTo[allNewNames[[j]],Rule@@@Transpose[{newNames,Range[oldIndex,oldIndex+\[Delta]]}]];
            newShape=Join[newShape[[;;oldIndex-1]],newDimensions,newShape[[oldIndex+1;;]]];
          ]
        ]
      ],
      {which,{{rowSplitting,1,rowNames},{colSplitting,2,colNames}}},
      {splitting,which[[1]]}
    ];
    NamedTensor[Sequence@@allNewNames,ArrayReshape[data,newShape]]
  ];
SplitIndices[t_NamedTensor,rowSplitting_Association,colSplitting_Association]:=SplitIndices[t,Normal@rowSplitting,Normal@colSplitting];
SplitIndices[t_NamedTensor,splitting__Rule]:=SplitIndices[t,{splitting},{splitting}];
SplitIndices`normalizable[_String->_Association]:=True;
SplitIndices`normalize[key_String->value_Association]:=key->Normal@value;
SplitIndices`normalizable[_String->_List?(AnyTrue[Head[#]===String||Head[#]===Integer&])]:=True;
SplitIndices`normalize[key_String->value_List]:=key->MapIndexed[Switch[#1,_String,#1->-1,_Integer,key<>"."<>ToString[#2[[1]]]->#1,_,#1]&,value];
SplitIndices`normalizable[_String->_Integer?Positive]:=True;
SplitIndices`normalize[key_String->value_Integer?Positive]:=key->Array[key<>"."<>ToString[#]->-1&,value];
SplitIndices[t_NamedTensor,rowSplitting_List,colSplitting_List]/;
  VectorQ[rowSplitting,SplitIndices`normalizable]||VectorQ[colSplitting,SplitIndices`normalizable]:=
  SplitIndices[t,SplitIndices`normalize/@rowSplitting,SplitIndices`normalize/@colSplitting];
SplitRowIndices[t_NamedTensor,splitting__Rule]:=SplitIndices[t,{splitting},{}];
SplitRowIndices[t_NamedTensor,splitting_Association]:=SplitIndices[t,splitting,<||>];
SplitRowIndices[t_NamedTensor,splitting_List]:=SplitIndices[t,splitting,{}];
SplitColumnIndices[t_NamedTensor,splitting__Rule]:=SplitIndices[t,{},{splitting}];
SplitColumnIndices[t_NamedTensor,splitting_Association]:=SplitIndices[t,<||>,splitting];
SplitColumnIndices[t_NamedTensor,splitting_List]:=SplitIndices[t,{},splitting];
Protect[SplitRowIndices,SplitColumnIndices,SplitIndices];
